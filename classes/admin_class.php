...
/* -------------admin_password_change------------*/
	...
	public function add_new_task($data){
		...
		public function update_task_info($data, $task_id, $user_role ){
			...
			$time_completed =  date ('Y-m-d H:i:s', time() );
			...
			try{
				if($user_role == 1){
				$update_task = $this->db->prepare("UPDATE task_info SET t_title = :x, t_description = :y, t_start_time = :z, t_end_time = :a, t_user_id = :b,  time_completed = :d, status = :c WHERE task_id = :id ");
				...
				$update_task->bindparam(':d', $time_completed);

				
				}else{
					$update_task = $this->db->prepare("UPDATE task_info SET t_title = :x, t_description = :y,  t_user_id = :b,  time_completed = :d, status = :c WHERE task_id = :id ");
